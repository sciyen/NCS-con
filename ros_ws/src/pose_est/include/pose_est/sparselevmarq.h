...
//    University of Denmark, {DTU}", address      = "Richard Petersens Plads,
//    Building 321, {DK-}2800 Kgs. Lyngby", url          =
//    "http://www.ltu.se/cms_fs/1.51590!/nonlinear_least_squares.pdf"
//}
template <typename T>
class SparseLevMarq {
   public:
    struct Params {
        Params() {}
        Params(int _maxIters, T _minError, T _min_step_error_diff = 0,
               T _min_average_step_error_diff = 0.001, T _tau = 1,
               T _der_epsilon = 1e-3) {
            maxIters = _maxIters;
            minError = _minError;
        ...
        T min_step_error_diff = 0;  // if error reduction between iterations is
                                    // below this, optimization stops
        T min_average_step_error_diff = 0.001;
        T tau = 1;  // indicating how near the initial solution is estimated to
                    // be to the real one. If 1, it means that it is very far
                    // and the first
        T der_epsilon = 1e-3;  // value employed for automatic differentiation
        bool cal_dev_parallel =
            true;  // indicates if the compuation of derivatives is done in
                   // parallel. If so, the error function must be reentrant
        bool use_omp = true;
        bool verbose = false;
    };
    ...
    typedef std::function<void(const eVector &, Eigen::SparseMatrix<T> &)>
        F_z_J;
    SparseLevMarq();
     ...
     * @param min_step_error_diff minimum error difference between two
     * iterations. If below this level, then stop.
     * @param tau parameter indicating how near the initial solution is
     * estimated to be to the real one. If 1, it means that it is very far and
     * the first
     * @param der_epsilon increment to calculate the derivate of the evaluation
     * function step will be very short. If near 0, means the opposite. This
     * value is auto calculated in the subsequent iterations.
     */
    void setParams(int maxIters, T minError, T min_step_error_diff = 0,
                   T tau = 1, T der_epsilon = 1e-3);
    void setParams(const Params &p);

    /**
     * @brief solve  non linear minimization problem ||F(z)||, where F(z)=f(z)
     * f(z)^t
     * @param z  function params 1xP to be estimated. input-output. Contains the
     * result of the optimization
     * @param f_z_x evaluation function  f(z)=x
     *          first parameter : z :  input. Data is in T precision as a row
     * vector (1xp) second parameter : x :  output. Data must be returned in T
     * @param f_J  computes the jacobian of f(z)
     *          first parameter : z :  input. Data is in T precision as a row
     * vector (1xp) second parameter : J :  output. Data must be returned in T
     * @return final error
     */
    ...
    // sets a function that indicates when the algorithm must be stop. returns
    // true if must stop and false otherwise
    void setStopFunction(std::function<bool(const eVector &)> stop_function) {
        _stopFunction = stop_function;
    }

    void calcDerivates_omp(const eVector &z, Eigen::SparseMatrix<T> &sJ,
                           F_z_x f_z_x);
    void calcDerivates(const eVector &z, Eigen::SparseMatrix<T> &sJ,
                       F_z_x f_z_x);
    Params _params;
   ...
   private:
    ...
    void add_missing_diagonal_elements(Eigen::SparseMatrix<T> &M) throw(
        std::exception);
    void get_diagonal_elements_refs_and_add(Eigen::SparseMatrix<T> &M,
                                            std::vector<T *> &d_refs,
                                            T add_val) throw(std::exception);
    void mult(const Eigen::SparseMatrix<T> &lhs,
              const Eigen::SparseMatrix<T> &rhs, Eigen::SparseMatrix<T> &res);
};
template <typename T>
 ...
 * @param maxIters maximum number of iterations of the algoritm
 * @param minError to stop the algorithm before reaching the max iterations
 * @param min_step_error_diff minimum error difference between two iterations.
 * If below this level, then stop.
 * @param tau parameter indicating how near the initial solution is estimated to
 * be to the real one. If 1, it means that it is very far and the first
 * @param der_epsilon increment to calculate the derivate of the evaluation
 * function step will be very short. If near 0, means the opposite. This value
...
template <typename T>
void SparseLevMarq<T>::calcDerivates_omp(const eVector &z,
                                         Eigen::SparseMatrix<T> &sJ,
                                         F_z_x f_z_x) {
    std::vector<std::vector<Eigen::Triplet<T>>> sp_triplets(
        omp_get_max_threads());
#pragma omp parallel for
    ...
    for (size_t i = 0; i < sp_triplets.size(); i++) {
        memcpy(&sp_tripletsAll[cidx], &sp_triplets[i][0],
               sizeof(Eigen::Triplet<T>) * sp_triplets[i].size());
...
template <typename T>
void SparseLevMarq<T>::calcDerivates(const eVector &z,
                                     Eigen::SparseMatrix<T> &sJ, F_z_x f_z_x) {
...
T SparseLevMarq<T>::solve(eVector &z, F_z_x f_z_x) throw(std::exception) {
    if (_params.cal_dev_parallel && _params.use_omp)
        return solve(
            z, f_z_x,
            std::bind(&SparseLevMarq<T>::calcDerivates_omp, this,
                      std::placeholders::_1, std::placeholders::_2, f_z_x));
    else
        return solve(
            z, f_z_x,
            std::bind(&SparseLevMarq<T>::calcDerivates, this,
                      std::placeholders::_1, std::placeholders::_2, f_z_x));
}
template <typename T>
bool SparseLevMarq<T>::step(F_z_x f_z_x) throw(std::exception) {
    if (_params.cal_dev_parallel && _params.use_omp)
        return step(f_z_x, std::bind(&SparseLevMarq<T>::calcDerivates_omp, this,
                                     std::placeholders::_1,
                                     std::placeholders::_2, f_z_x));
    else
        return step(f_z_x, std::bind(&SparseLevMarq<T>::calcDerivates, this,
                                     std::placeholders::_1,
                                     std::placeholders::_2, f_z_x));
}
...
template <typename T>
void SparseLevMarq<T>::get_diagonal_elements_refs_and_add(
    Eigen::SparseMatrix<T> &M, std::vector<T *> &refs,
    T add_val) throw(std::exception) {
    refs.resize(M.cols());
...
template <typename T>
void SparseLevMarq<T>::mult(const Eigen::SparseMatrix<T> &lhs,
                            const Eigen::SparseMatrix<T> &rhs,
                            Eigen::SparseMatrix<T> &res) {
...
#pragma omp parallel for
    for (uint32_t j = 0; j < cols; ++j) {
        ...
        for (typename Eigen::SparseMatrix<T>::InnerIterator rhsIt(rhs, j);
             rhsIt; ++rhsIt) {
            T y = rhsIt.value();
            uint32_t k = rhsIt.index();
            // add all indices
            for (typename Eigen::SparseMatrix<T>::InnerIterator lhsIt(lhs, k);
                 lhsIt; ++lhsIt) {
                uint32_t i = lhsIt.index();
                T x = lhsIt.value();
                auto iter = row_val.find(i);
                if (iter == row_val.end())
                    row_val.insert(std::make_pair(i, x * y));
                else
                    iter->second += x * y;
            }
        }
    ...
    for (auto &omp_thread : omp_container)
        for (auto c_r_v : omp_thread) {  // take each thread results
            ...
            for (auto r_v : c_r_v.second)  // for each column element, add it
                resCol.insertBackByOuterInnerUnordered(j, r_v.first) =
                    r_v.second;
        }
    resCol.finalize();
...
template <typename T>
void SparseLevMarq<T>::add_missing_diagonal_elements(
    Eigen::SparseMatrix<T> &M) throw(std::exception) {
...
#define splm_get_time(a, b) \
    std::chrono::duration_cast<std::chrono::duration<T>>(a - b).count()
template <typename T>
bool SparseLevMarq<T>::step(F_z_x f_z_x, F_z_J f_J) throw(std::exception) {
    ...
    if (mu < 0) {  // first time only
        ...
        for (int k = 0; k < JtJ.outerSize(); ++k)
            for (typename Eigen::SparseMatrix<T>::InnerIterator it(JtJ, k); it;
                 ++it)
                if (it.row() == it.col())
...
#if 1  // very efficient in any case, but particularly if initial dump does not
       // produce improvement and must reenter
        if (refs.size() == 0) {  // first time into the do
            ...
            for (size_t i = 0; i < refs.size(); i++)
                *refs[i] += mu - prev_mu;  // update mu
        prev_mu = mu;

        Eigen::SimplicialLDLT<Eigen::SparseMatrix<T>> chol(
            JtJ);  // performs a Cholesky
#else              // less efficient, but easier to understand
        Eigen::SparseMatrix<T> A = JtJ + I * mu;
        Eigen::SimplicialLDLT<Eigen::SparseMatrix<T>> chol(
            A);  // performs a Cholesky
#endif
        t5 = std::chrono::high_resolution_clock::now();
    ...
    if (_params.verbose)
        std::cout << std::setprecision(5) << "Curr Error=" << currErr
                  << " AErr(prev-curr)=" << (prevErr - currErr) / x64.rows()
                  << " gain=" << gain << " dumping factor=" << mu << std::endl;

    if (_params.verbose) {
        std::cerr << " J=" << splm_get_time(t2, t1)
                  << " transpose=" << splm_get_time(t22, t2)
                  << " Jt*J=" << splm_get_time(t3, t22)
                  << " B=" << splm_get_time(t4, t3)
                  << " chol=" << splm_get_time(t6, t5) << std::endl;
...
T SparseLevMarq<T>::solve(eVector &z, F_z_x f_z_x,
    ...
    if (_stopFunction) {
        ...
        for (int i = 0; i < _params.maxIters && !mustExit; i++) {
            if (_params.verbose)
                std::cerr << "iteration " << i << "/" << _params.maxIters
                          << "  ";
            bool isStepAccepted = step(f_z_x, f_J);
            // check if we must exit
            if (currErr < _params.minError) mustExit = 1;
            if (fabs(prevErr - currErr) <= _params.min_step_error_diff ||
                fabs((prevErr - currErr) / x64.rows()) <=
                    _params.min_average_step_error_diff ||
                !isStepAccepted)
                mustExit = 2;
