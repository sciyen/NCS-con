#include "pose_est/cam_config.h"
#include <iostream>
#include "pose_est/filesystem.h"
...
void CamConfig::setCamMat(cv::Mat c_mat) {
    if (c_mat.rows != 3 || c_mat.cols != 3)
        throw std::runtime_error(
            "The size of the input camera matrix must be 4x4!");
...
void CamConfig::setDistCoeffs(cv::Mat d_coeffs) {
    if (d_coeffs.rows != 1 && d_coeffs.cols != 1)
        throw std::runtime_error(
            "The size of the input matrix for distorsion coefficients must be "
            "1xN nor Nx1!");
...
void CamConfig::setImageSize(cv::Size im_size) { image_size = im_size; }

const cv::Mat& CamConfig::getCamMat() const { return cam_mat; }

const cv::Mat& CamConfig::getDistCoeffs() const { return dist_coeffs; }

const cv::Size& CamConfig::getImageSize() const { return image_size; }
...
bool CamConfig::read_from_file(std::string path) {
    ...
    if (file["matrix_coefficients"].isNone())
        return false;
    else
        file["matrix_coefficients"] >> cam_mat;
