...
#include "pose_est/include/aruco/aruco.h"
#include "pose_est/include/dataset.h"
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (size_t i = 0; i < num_cams; i++) {
            detector[i].getParameters().setDetectionMode(
                aruco::DetectionMode::DM_VIDEO_FAST, 0);
            detector[i].setDictionary(dictionary_name);
            detector[i].getParameters().setCornerRefinementMethod(
                aruco::CornerRefinementMethod::CORNER_LINES);
        }
        ...
        for (size_t frame_num : frame_nums) {
            ...
            for (size_t cam = 0; cam < num_cams; cam++) {
                ...
                for (size_t m = 0; m < cam_markers[cam].size(); m++)
                    if (markers_count.find(cam_markers[cam][m].id) !=
                        markers_count.end())
            ...
            for (size_t cam = 0; cam < num_cams; cam++) {
                ...
                for (size_t m = 0; m < markers.size(); m++) {
                    if (markers_count[markers[m].id] >=
                        min_detections_per_marker)  // if there is more that one
                                                    // instance of that marker
            ...
            for (size_t cam = 0; cam < num_cams; cam++) {
                ...
                output_file.write((char*)&new_markers_size,
                                  sizeof(new_markers_size));

                for (size_t m = 0; m < new_markers_size; m++) {
                    marker_ids.insert(new_markers[cam][m].id);
                    ArucoSerdes::serialize_marker(new_markers[cam][m],
                                                  output_file);
